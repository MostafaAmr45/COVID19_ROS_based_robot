...
######################################################
    ...
    #####################################################
        ...
        rospy.Subscriber("rwheel_ticks", Int16, self.wheelCallback) 
        rospy.Subscriber("rwheel_vtarget", Float32, self.targetCallback) 
        self.pub_motor = rospy.Publisher('rmotor',Float32, queue_size=10) 
        self.pub_vel = rospy.Publisher('rwheel_vactual', Float32, queue_size=10)
    ...
    #####################################################
        ...
        while not rospy.is_shutdown():
            ...
            # rospy.loginfo("-D- %s **** wheel updated enc=%0.3f **** " % (self.nodename, self.prev_encoder))
            # rospy.loginfo("-D- %s **** wheel updated wheel=%0.3f **** " % (self.nodename, self.wheel_mult))
            self.r.sleep()
